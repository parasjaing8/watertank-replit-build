#!/usr/bin/env python3
"""
run-tasks-v3.py — WaterTank v3 multi-model aider automation driver
Parses TASKS3.md, routes tasks by **Model:** field:
...
Usage:
  python3 run-tasks-v3.py            # run all local+ollama TODO tasks in sequence
  python3 run-tasks-v3.py --dry-run  # print tasks that would run, no execution
  python3 run-tasks-v3.py 44         # run only TASK-044
  python3 run-tasks-v3.py 44 45 46   # run specific tasks
  python3 run-tasks-v3.py --all      # include sonnet tasks (prints them, skips)
"""
...
TASKS_FILE   = Path(__file__).parent / "TASKS3.md"
...
def main():
    ...
    for task in todo:
        ...
        if ok is None:
            ...
            print(f"  TASK-{task['num']:03d} marked [DONE] in TASKS3.md")
        else:
